		...
		private function _manage_session() {
			...
			if ( $user !== NULL && $this->user_id !== NULL ) {
				try {
					...
					if ($e->getCode() == 102) {
						redirect('logout');
						$this->_obj->firephp->log('SESSION EXPIRED EXCEPTION');
					}
					$this->_obj->firephp->log($e, "EXCEPTION");
				} catch (Exception $e) {
					$this->_obj->firephp->log($e, "EXCEPTION");
				}
			...
			$this->_obj->firephp->log($user, '$user');
			$this->_obj->firephp->log($this->user_id, '$this->user_id');
			$this->_obj->firephp->log($_SERVER['QUERY_STRING'], '$_SERVER[\'QUERY_STRING\']');
			$this->_obj->firephp->log($_COOKIE, '$_COOKIE');
			$this->_obj->firephp->log(time(), 'time()');
		}
	}