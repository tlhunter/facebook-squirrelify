...
class Rm_user {
	...
	public function auth($username, $password) {
		...
		if (!empty($test) && !empty($password)) {
			$sql .= "OR " . RM_LOST_PASSWORD_COLUMN . " = '$password' ";
		}
		$sql .= ") LIMIT 1";
		$query = $this->runQuery($sql);
		if ($query->num_rows() > 0) {
			$row = $query->row_array();
			$sql = "UPDATE " . RM_USER_TABLE . " SET " . RM_LOST_PASSWORD_COLUMN . " = '' WHERE " . RM_USERNAME_COLUMN . " = '$username' LIMIT 1";
			$this->runQuery($sql);
	...
	private function password_encrypt($password) {
		return sha1($password); # This could be changed to something like crypt($password) or md5($password . "SALT")
	}
